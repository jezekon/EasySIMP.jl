...
- `densities`: Vector of density values for each cell
- `cache`: Element stiffness cache (or nothing)
- `ke_buffer`: Pre-allocated element stiffness matrix buffer
- `fe_buffer`: Pre-allocated element force vector buffer
"""
...
"""
    assemble_stiffness_matrix_simp!(K, f, dh, cellvalues, material_model, densities, cache=nothing)

Backward-compatible version that auto-allocates buffers.
"""
function assemble_stiffness_matrix_simp!(
    K,
    f,
