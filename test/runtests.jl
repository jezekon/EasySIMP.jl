...
@testset "EasySIMP.jl Tests" begin
  ...
  if RUN_BEAM_fixed
    @testset "Cantilever Beam SIMP (fixed)" begin
      ...
      if isempty(force_nodes)
        ...
        min_dist = Inf
        closest_node = 1
        for node_id in 1:getnnodes(grid)
      ...
      opt_params = OptimizationParameters(
        ...
        tolerance = 0.010,          # ← Menší tolerance
        filter_radius = 2.0,
  ...
  if RUN_CHAPADLO
    @testset "Chapadlo SIMP Optimization" begin
      ...
      fixed_nodes =
        select_nodes_by_circle(grid, [0.0, 75.0, 115.0], [0.0, -1.0, 0.0], 16.11, 1e-3)
      ...
      opt_params = OptimizationParameters(
        ...
        p = 3.0,
        volume_fraction = 0.3,    # 30% objemový poměr
        max_iterations = 2000,                  # Více iterací pro komplexnější geometrii
        tolerance = 0.010,
